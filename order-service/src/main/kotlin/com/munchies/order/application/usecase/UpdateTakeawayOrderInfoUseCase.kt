...
class UpdateTakeawayOrderInfoUseCase(private val repository: OrderRepository) :
  ...
  override fun execute(command: UpdateTakeawayOrderCommand): UpdateTakeawayOrderInfo.Result {
    ...
    return when {
      ...
      command.pickupTime <= System.currentTimeMillis() -> InvalidDate
      else -> {
        val updatedOrder = (order as TakeawayOrder).copy(
