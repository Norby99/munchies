...
export class GetRestaurantRoute
  extends GetRestaurantAPI
  implements SimpleRoute<GetRestaurantResponse>
{
  path = this.getPath();
  ...
  async getRestaurant(restaurantId: string): Promise<GetRestaurantResponse> {
  ...
  forward = async (req: AuthedRequest): Promise<GetRestaurantResponse | ErrorResponse> => {
    try { return await this.getRestaurant(req.params.restaurantId); }
    catch (error: any) {
