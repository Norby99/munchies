...
export class GetRestaurantMenusRoute
  extends GetRestaurantMenusAPI<
    GetRestaurantMenusResponse | ErrorResponse
  >
  implements SimpleRoute<GetRestaurantMenusResponse>
{
  ...
  async getRestaurantMenus(
    restaurantId: string,
  ): Promise<GetRestaurantMenusResponse | ErrorResponse> {
    ...
    const response = request<GetRestaurantMenusResponse>(
  ...
  private handler: {
    ...
    forward: async (req: AuthedRequest) => {
      try {
        return this.getRestaurantMenus(req.params.restaurantId);
      } catch (err: any) {
        return new ErrorResponse(
          "GetRestaurantMenus forward: \n" + String(err),
          500,
        );
      }
    },
