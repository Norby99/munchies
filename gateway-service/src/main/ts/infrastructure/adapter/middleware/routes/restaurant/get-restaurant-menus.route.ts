...
export class GetRestaurantMenusRoute
  extends GetRestaurantMenusAPI
  implements SimpleRoute<GetRestaurantMenusResponse>
{
  path = this.getPath();
  ...
  async getRestaurantMenus(restaurantId: string): Promise<GetRestaurantMenusResponse> {
  ...
  forward = async (req: AuthedRequest): Promise<GetRestaurantMenusResponse | ErrorResponse> => {
    try { return await this.getRestaurantMenus(req.params.restaurantId); }
    catch (error: any) {
